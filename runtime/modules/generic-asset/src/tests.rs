...
fn self_transfer_should_fail() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                GenericAsset::free_balance(&asset_id, &1),
...
fn repatriate_reserved_return_amount_subtracted_by_slash_amount() {
    ExtBuilder::default().build().execute_with(|| {
        GenericAsset::set_reserved_balance(&1, &0, 100);
        assert_eq!(
            GenericAsset::repatriate_reserved(&1, &0, &1, 130, BalanceStatus::Free),
            30
        );
    });
}
...
fn repatriate_reserved_return_none() {
    ExtBuilder::default().build().execute_with(|| {
        GenericAsset::set_reserved_balance(&1, &0, 100);
        assert_eq!(
            GenericAsset::repatriate_reserved(&1, &0, &1, 90, BalanceStatus::Free),
            0
        );
    });
}
...
fn mint_should_increase_asset() {
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            assert_eq!(
                GenericAsset::free_balance(&asset_id, &origin),
                initial_issuance
            );
        });
}
...
fn update_permission_should_raise_event() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let expected_event = TestEvent::generic_asset(RawEvent::PermissionUpdated(
                asset_id,
                permissions.clone(),
            ));
            // Assert
...
fn burn_should_raise_event() {
    ...
    ExtBuilder::default()
        ...
        .execute_with(|| {
            ...
            let expected_event =
                TestEvent::generic_asset(RawEvent::Burned(asset_id, origin, amount));
