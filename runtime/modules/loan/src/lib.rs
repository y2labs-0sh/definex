...
decl_storage! {
    trait Store for Module<T: Trait> as Loan {
        ...
        pub ActiveLoanPackages get(active_loan_packages) : linked_map hasher(blake2_256) LoanPackageId => LoanPackage<T::Balance, T::AssetId>;
        /// all packages including both the active and the inactive
        pub LoanPackages get(loan_package) : linked_map hasher(blake2_256) LoanPackageId => LoanPackage<T::Balance, T::AssetId>;
        /// increase monotonically
        NextLoanId get(next_loan_id) config() : LoanId;
        /// currently running loans
        pub Loans get(get_loan_by_id) : linked_map hasher(blake2_256) LoanId => Loan<T::AccountId, T::Balance, T::Moment>;
        ...
        /// total balance of profit that has gained from fees and penaltys
...
impl<T: Trait> Module<T> {
    ...
    pub fn disable_loan_package(package_id: LoanPackageId) -> DispatchResult {
            ...
            <LoanPackages<T>>::contains_key(package_id),
            "invalid package id"
        );
    ...
    pub fn repay_loan(who: T::AccountId, loan_id: LoanId) -> DispatchResult {
        ...
        let loan = <Loans<T>>::get(loan_id);
            ...
            <LoanPackages<T>>::contains_key(loan.package_id),
    ...
    pub fn apply_for_loan(
        ...
        loan_amount: T::Balance,
            ...
            <ActiveLoanPackages<T>>::contains_key(package_id),
            "invalid package id"
        );
    ...
    fn on_each_block(_height: T::BlockNumber) {
        ...
        for (loan_id, loan) in <Loans<T>>::enumerate() {
            ...
            match Self::check_loan_health(&loan, now, btc_price, liquidation_thd, warning_thd) {
                ...
                LoanHealth::Extended => {
                    if loan.status != LoanHealth::Extended {
                ...
                LoanHealth::Expired => {
                    ...
                    if new_ltv >= liquidation_thd {
                        ...
                        Self::deposit_event(RawEvent::Expired(
                            loan_id,
                            who,
                            total_penalty,
                            total_interest,
            ...
            .and_then(|_| {
                <TotalCollateral<T>>::mutate(|v| {
                    match v.checked_sub(&(total_interest + total_penalty)) {
                        ...
                        None => Err(Error::<T>::TotalCollateralUnderflow)?,
                    }
                })
            })
            .unwrap_or_default();
