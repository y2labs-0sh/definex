...
decl_storage! {
    trait Store for Module<T: Trait> as LSBiding {
        ...
        pub Borrows get(borrows) : linked_map hasher(blake2_256) BorrowId => Borrow<T::AssetId, T::Balance, T::BlockNumber, T::AccountId>;
        pub BorrowIdsByAccountId get(borrow_ids_by_account_id) : map hasher(blake2_256) T::AccountId => Vec<BorrowId>;
