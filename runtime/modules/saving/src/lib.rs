...
where
    ...
    AssetId: Encode + Decode + Parameter + Member + AtLeast32Bit + Default + Copy,
    AccountId: Encode
        + Decode
        + Parameter
        + Member
...
decl_storage! {
    trait Store for Module<T: Trait> as Saving {
        ...
        ShareAssetCollected get(share_asset_collected) : map hasher(blake2_256) T::AccountId => T::Balance;

        /// each phase has its own asset, "IOUAsset"
        /// this holds a map as "IOUAsset" => "Phase"
        IOUAssetPhaseId get(iou_asset_phase_id) : map hasher(blake2_256) T::AssetId => PhaseId;
        ...
        }) : linked_map hasher(blake2_256) PhaseId => PhaseInfo<T::Balance, T::AssetId>;
        ...
        ShareUnreleasedList get(account_future_releases) : linked_map hasher(blake2_256) T::AccountId => Vec<ShareReleasePack<T::Balance, T::AssetId, T::AccountId>>;

        /// tracking share asset movement
        AccountShares get(account_shares) : linked_map hasher(blake2_256) T::AccountId => T::Balance;
        ...
        /// Mint RIO to this account when saving
...
decl_error! {
    pub enum Error for Module<T: Trait> {
        NotEnoughShareAsset,
        NotEnoughLockedBalance,
        ExceedAllowedRedeemLimit,
        SavingBalanceZero,
    }
}
...
decl_module! {
    pub struct Module<T: Trait> for enum Call where origin: T::Origin {
        ...
        pub fn set_share_asset_id(origin, asset_id: T::AssetId) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(<assets::Module<T>>::asset_exists(&asset_id), "invalid asset id for rio saving share asset");
        ...
        pub fn set_iou_asset_id_for_phase(origin, phase_id: PhaseId, asset_id: T::AssetId) -> DispatchResult {
            ensure_root(origin)?;
            ensure!(<PhaseInfos<T>>::contains_key(&phase_id), "invalid phase id for rio saving");
            ensure!(<assets::Module<T>>::asset_exists(&asset_id), "invalid iou asset id for rio saving");
            if <IOUAssetPhaseId<T>>::contains_key(&asset_id) {
        ...
        pub fn redeem(origin, iou_asset_id: T::AssetId, iou_asset_amount: T::Balance) -> DispatchResult {
            ...
            ensure!(<IOUAssetPhaseId<T>>::contains_key(&iou_asset_id), "no such contract assets");
...
impl<T: Trait> Module<T> {
    ...
    pub fn check_can_redeem(
        ...
        if !share_asset_id.is_zero() {
            ...
            if free_share_asset + returned_share_asset < free_share_asset_required {
                ...
                {
                    return Err(Error::<T>::NotEnoughShareAsset)?;
                }
            }

            if required_share_asset > returned_share_asset + unreleased + free_share_asset {
                return Err(Error::<T>::ExceedAllowedRedeemLimit)?;
            }
        }
        ...
        if change.is_none() {
            ...
            if change.is_positive() {
                ...
                new_list.push(aggregated);
                    ...
                    unreleased_total_balance >= change.get_value(),
                    Error::<T>::NotEnoughLockedBalance
                );
    ...
    pub fn create_staking(
        ...
        let phase_id = Self::current_phase_id();
            ...
            <PhaseInfos<T>>::contains_key(&phase_id),
    ...
    /// create_iou is called when a user deposit his BTC into rio saving program
    fn create_iou(
        owner: T::AccountId,
    ...
    fn register_share_pack(share_pack: ShareReleasePack<T::Balance, T::AssetId, T::AccountId>) {
        if !share_pack.is_empty() {
            ...
            if !<ShareUnreleasedList<T>>::contains_key(&owner) {
    ...
    /// create_release_pack_for_saving is called when a user deposit his BTC into rio saving program
    fn create_release_pack_for_saving(
        owner: T::AccountId,
        asset_id: T::AssetId,
        balance: T::Balance,
        phase_info: &PhaseInfo<T::Balance, T::AssetId>,
    ) -> Result<ShareReleasePack<T::Balance, T::AssetId, T::AccountId>, DispatchError> {
        if balance.is_zero() {
            return Err(Error::<T>::SavingBalanceZero)?;
        }
        ...
        let effective_phases_count = NumOfPhasesLeft::get();
            ...
            effective_phases_count >= 1,
            "should not create share packages for the last phase"
        );
    ...
    fn shift_next_phase(current_phase_id: PhaseId) -> Option<PhaseId> {
        ...
        if <PhaseInfos<T>>::contains_key(new_phase_id) {
    ...
    fn track_share_asset_movement(
        ...
        if *to != collection_account_id {
            if <AccountShares<T>>::contains_key(to) {
    ...
    for Module<T>
{
    fn before_asset_transfer(
        ...
        if <IOUAssetPhaseId<T>>::contains_key(&asset_id) && *to == Self::collection_account_id() {
...
impl<T: Trait> assets::traits::OnAssetTransfer<T::AssetId, T::AccountId, T::Balance> for Module<T> {
    fn on_asset_transfer(
        ...
        if *asset_id == collection_asset_id && *to == collection_account_id {
            return Self::create_staking(from.clone(), *balance)
                .map(|_: T::Balance| {})
                .map_err(|err| DispatchError::from(err));
        }

        // when user successfully wire some iou asset into our collection account
        // we consider this is a redeemal
        if <IOUAssetPhaseId<T>>::contains_key(asset_id) && *to == collection_account_id {
