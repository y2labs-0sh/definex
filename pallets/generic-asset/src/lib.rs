...
decl_storage! {
    trait Store for Module<T: Trait> as GenericAsset {
        ...
        pub FreeBalance:
            double_map hasher(opaque_blake2_256) T::AssetId, hasher(opaque_twox_128) T::AccountId => T::Balance;
        ...
        pub Symbols get(symbols) config() : map hasher(twox_64_concat) T::AssetId => Vec<u8>;
        ...
        pub Permissions get(fn get_permission):
            map hasher(opaque_blake2_256) T::AssetId => PermissionVersions<T::AccountId>;

        /// Any liquidity locks on some account balances.
        pub NextLockId get(fn next_lock_id) : u128 = 1;
        pub Locks get(fn locks):
            double_map hasher(opaque_blake2_256) LockIdentifier, hasher(opaque_blake2_256) T::AccountId => Vec<BalanceLock<T::Balance, T::AssetId>>;
    }
